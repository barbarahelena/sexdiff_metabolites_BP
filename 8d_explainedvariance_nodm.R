...
for(g in groups){
    for(d in c("Male", "Female")){
        li <- list.files(path = file.path(d, g))
        a <- str_detect(li, regex(g, ignore_case = T))
        b<- !(str_detect(li, 'PERMUTED'))
        ev_list[[g]][[d]] <- rio::import(file.path(d, g, li[which(a&b)], 'aggregated_metrics_regression.txt'))
    }
}
...
write.table(df, "221026_expvar_nodm.csv", sep=",")
clipr::write_clip(df)
...
for(g in groups){
    for(d in c("Male", "Female")){
        ...
        a <- str_detect(li, regex(g, ignore_case = T))
        b <- !(str_detect(li, 'PERMUTED'))
        ev_list2[[g]][[d]] <- rio::import(file.path(d,g, li[which(a&b)], 'model_results_per_iteration.txt'))
    }
}
...
(pl3 <- df4 %>% 
        ...
        annotate("rect", xmin=-Inf, xmax=Inf, 
                 ymin=0.0, ymax=-25, alpha=0.5, fill="grey")+
        geom_half_violin(side = "r", scale = "width", alpha = 0.5) +
...
ggsave("221026_expvar_bp_median.pdf", width = 6, height = 7)

ann_text <- data.frame()
for(g in levels(df3$group)){
    for(d in levels(df3$sex)){
        b <- data.frame(sex = d, outcome = g,lab = df3$mean_ev, ev = 30)
        ann_text <- rbind(ann_text,b)
    }
}
pl3 + geom_text(data = ann_text, mapping = aes(label = lab))


# for(i in 1:length(gem$mean_ev)){
#     pl3 <- pl3 + annotation_custom(
#         grob = textGrob(label = gem$mean_ev[i], hjust = 0, gp = gpar(cex = 0.8)),
#         xmin = levels(df4$outcome)[i],      # Vertical position of the textGrob
#         xmax = levels(df4$outcome)[i],
#         ymin = max(df4$`Explained Variance`[which(levels(df4$outcome)[i] == df4$outcome)])+0.03,
#         ymax = max(df4$`Explained Variance`[which(levels(df4$outcome)[i] == df4$outcome)])+0.03)
# }

ggsave("results/220319_Expvar.pdf")

(pl4 <- ggplot(df4, aes(x = sex, y = ev, color = sex, fill = sex))+
        annotate("rect", xmin=-Inf, xmax=Inf, 
                 ymin=0.0, ymax=-30, alpha=0.5, fill="grey")+
        geom_hline(yintercept=0.0, linetype="dashed")+
        geom_violin(scale = "width", alpha = 0.4) +
        ggbeeswarm2::geom_beeswarm(corral.width = 0.6, alpha = 0.5,
                      method = "hex")+
        labs(title = 'Machine learning models: explained variance',
             x = '', y = 'Explained variance', linetype = '', size = '') +
...
for (i in 1:length(gem$mean_ev))  {
    pl4 <- pl4 + annotation_custom(
        grob = textGrob(label = gem$mean_ev[i], hjust = 0, gp = gpar(cex = 0.8)),
        xmin = rev(levels(df4$sex))[i],      # Vertical position of the textGrob
        xmax = rev(levels(df4$sex))[i],
        ymin = 0,
        ymax = 30)
}
pl4
...
for (i in c(1)) {
    for(d in c("Young")){
        ...
        mean_ev <- mean(ev_list2[[i]][[d]]$`Explained Variance`)
        sd_ev <- sd(ev_list2[[i]][[d]]$`Explained Variance`)
        row <- cbind(group, sex, mean_ev, sd_ev)
        df2<- rbind(df2, row)
    }
}
