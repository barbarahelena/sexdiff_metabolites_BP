...
d_male <- df %>% filter(!is.na(SDNN)) %>% filter(Sex == "Male")
d_female <- df %>% filter(!is.na(SDNN)) %>% filter(Sex == "Female")

#### Make input data for ML model - Males ####
...
# make input data for SDNN - Males
path <- 'SDNN/Male'
dir.create(path)
write_data(df_met_male, file.path(path, 'input_data'))
y <- as.data.frame(d_male$logSDNN)
...
# make input data for SDNN - Females
path <- 'SDNN/Female'
dir.create(path)
write_data(d_met_female, file.path(path, 'input_data'))
y <- as.data.frame(d_female$logSDNN)
write_y(y, name_y = 'y_reg.txt', file.path(path, 'input_data'))
