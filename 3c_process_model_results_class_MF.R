...
plot_feature_importance <- function(path_true, top_n){
    ...
    pl <- ggplot(data=r, aes(y=RelFeatImp, x=Tax, fill=RelFeatImp)) + 
        ...
        coord_flip() +
        ylab("Metabolite Relative Importance (%)")+
        xlab("") +
...
plot_features_tests_class <- function(input_path, output_path, top_n=10){
    theme_Publication <- function(base_size=11, base_family="sans") {
        library(grid)
        library(ggthemes)
        (theme_foundation(base_size=base_size, base_family=base_family)
...
plot_feature_importance(path_true, 10)
(pl_featimp <- plot_feature_importance_colors(path_true, top_n = 15))
plot_features_tests_class(data_path, path_true, top_n=30)
plot_features_tests_top(data_path, path_true, top_n=3, nrow=1)
